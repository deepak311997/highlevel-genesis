 ...
 *
 * The endpoint hands back a URL and nothing else, which makes it easy to
 * under-test. What actually matters is who is allowed to obtain one: the URL
 * carries a `state` binding a Firebase uid, and the callback will write a
 * HighLevel connection against whatever uid that state names. So an endpoint
 * that minted one for an unauthenticated or unverified caller would let someone
...
describe('POST /api/hl/connect', () => {
  ...
  /*
   * The D26 case. This caller holds a genuine, unexpired token for a real
   * account — a router guard would never see them, because they need not use a
   * browser at all.
   */
