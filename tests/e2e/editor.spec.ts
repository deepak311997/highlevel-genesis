...
/**
 * AC-30 and AC-31 — **the only level where Monaco actually runs**.
 *
 * Everything decidable without a browser is proven cheaply elsewhere: the language map, the
 * append-vs-replace rule, the model registry, and the props `CodeEditor` passes. What is left
 * needs a layout engine and a canvas, and it is precisely the part that would ship broken and
 * silent:
 */
...
/**
 * AC-25's runtime half — **the editor never reaches for a CDN**.
 *
 * `no-cdn.spec.ts` proves nobody wrote one of these hosts into `frontend/src`, which is a claim
 * about the source and not about the running app: `@monaco-editor/loader`'s own default —
 * `cdn.jsdelivr.net/npm/monaco-editor@…` — is inside the bundle whatever our source says, and
 * what makes it inert is `loader.config({ monaco })` having run before the first `init()`. Only
 * a browser can be asked whether that held. A regression here is invisible on a developer's
 * machine and fatal on a locked-down network, which is exactly the shape of failure this suite
 * exists to catch.
 */
...
test.describe('Slice 07 — Monaco editor', () => {
  ...
  test('code streams coloured into a locked editor, and tabs keep their edits', async ({
    ...
    /*
     * Locked, and saying so — and **Monaco actually refuses the keystroke**, which is the half
     * no other level can assert: L2 proves the `readOnly` option is passed, not that it was
     * honoured.
     */
  ...
  test('the editor renders with a real height at both layouts', async ({ page }) => {
    ...
    /*
     * And it folds, which is what makes a rail affordable in a panel this narrow: the editor is
     * wider afterwards, and no row is left focusable behind a zero-width box.
     */
