...
/**
 * The HighLevel cheat-sheet — what the model is told, checked against the tables it was rendered
 * from (D2, AC-1 to AC-9).
 *
 * The point of every case below is that the cheat-sheet is **derived data**, not prose someone
 * typed once. A hand-written endpoint list drifts on the first allowlist change and the drift is
 * invisible: the model goes on confidently generating a route the proxy answers `403` on, and
 * nothing fails until a user runs the app. So the allowlist is checked in both directions (AC-1
 * names every enabled row, AC-2 refuses any path the proxy would), the error codes are read from
 * `proxyError.ts` rather than restated, and the response examples are checked field by field
 * against the payloads actually recorded from the sandbox.
 */
 ...
 *
 * `matchRoute` validates parameters against its own grammar, so a scan that fed it the literal
 * `:contactId` would be testing the grammar rather than the route table — `:` is not in
 * `[A-Za-z0-9_-]` and every parameterised row would come back `invalid_path`.
 */
...
describe('AC-1 — every enabled row appears, exactly as the proxy spells it', () => {
  /*
   * `method` and `pattern` verbatim, not a paraphrase: the proxy matches on the pattern, so
   * anything else in the prompt is a second spelling of a route and the model has no way to know
   * which one is real.
   */
  ...
  /* The other half of the same guarantee: the table names *only* rows that exist. */
  ...
  /*
   * A row added to the allowlist without a sentence about it renders {@link NOTE_MISSING} rather
   * than an empty dash.
   */
...
describe('AC-2 — every route it names is one the proxy would allow', () => {
  ...
  /*
   * A scan that finds nothing passes vacuously, which is the one way this case could be green
   * while the cheat-sheet named something absurd.
   */
  ...
  /*
   * The negatives that make the scan meaningful. Neither verb is on the allowlist, and `DELETE
   * /contacts/{id}` in a loop is the exact confused-deputy output `HIGHLEVEL_PLATFORM.md` §8
   * names — a cheat-sheet that mentioned it, even to forbid it, would be teaching the string.
   */
  ...
  /*
   * The hand-written half, which is the residual exposure R5 names and the half the scanners
   * above cannot see: both only find paths that are *already* verb-prefixed or inside an `hl(`
   * call, i.e.
   */
...
describe('AC-3 — the flagged row follows the environment', () => {
  /*
   * The cheat-sheet is rendered once, at module load, so the environment cannot be varied by
   * setting it and re-reading the constant.
   */
...
describe('AC-5 — locationId is never sent', () => {
  /*
   * Named as a key rather than described, because the model has to recognise the field it is
   * about to type.
   */
...
describe('AC-6 — the calendar time format, and the trap', () => {
  ...
  /*
   * The highest-value line in the cheat-sheet. ISO 8601 on `/calendars/events` does not error —
   * it answers HTTP 200 with an empty list, which is indistinguishable from a calendar with
   * nothing in it.
   */
...
describe('AC-7 — the error contract, read from proxyError.ts', () => {
  ...
  /*
   * Phrase-level, because the single words are free. `'try'` is already in `country` (a field of
   * the contacts example) and in `retrying`; `'catch'` is in the worked `} catch (err) {`;
   * `'message'` is in `showMessage` and in `/conversations/messages`.
   */
...
describe('AC-9 — every example field exists in the recorded payload', () => {
  ...
  /**
   * Every key reachable in a value: an object contributes its keys and then its values are
   * walked, an array's elements are walked.
   */
...
describe('the cheat-sheet teaches one function and no URL', () => {
  /*
   * AC-4 is asserted over the whole prompt in `prompt.spec.ts`; it is repeated here on the one
   * block that could plausibly carry a URL, so a mistake is reported against the file that made
   * it.
   */
