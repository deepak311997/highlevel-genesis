...
// for the whole API, unchanged since Slice 2 (D28) — and this router no longer
// has one: writing a message is `/generate`'s job, inside the request that
// streams the reply, so the attestation that guarded the write lives there.
messagesRouter.get(
