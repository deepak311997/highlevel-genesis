 ...
 *
 * Deliberately not a Firestore subscription: the connection document holds tokens
 * and is denied to every client, so this state comes from an endpoint — a
 * snapshot, refreshed on demand, rather than something that updates itself.
 */
 ...
 * recognise", which renders as an em dash. A surface that genuinely holds nothing
 * answers `0`, which is an honest empty state rather than a failure.
 */
export interface SurfaceProbe {
  count: number | null
  error: string | null
  /** This surface failed in a way only reconnecting fixes. */
  reconnect: boolean
}
...
/**
 * Narrowed by hand, defensively: `total` is HighLevel's field and this slice
 * forwards their body verbatim, so a shape change upstream has to render as "—"
 * rather than as `NaN` in a panel nobody can debug from.
 */
...
/**
 * The three probe calls: one per surface, the smallest payload each will return,
 * and no parameter beyond the injected `locationId`.
 *
 * Deliberately not `/calendars/events`, which needs a calendar id and a time
 * window — its failure would be ambiguous between "no access" and "no events this
 * fortnight", which is the one thing this section exists to answer unambiguously.
 */
const SURFACES = [
  {
    key: 'contacts',
 ...
 * something. On the one screen whose purpose is diagnosing a HighLevel call,
 * "Could not read contacts." is a shrug and "Could not read contacts. (Invalid
 * JWT)" is a fix.
 *
 * Absent, empty, or merely repeating the message earns no parentheses.
 */
...
export const useHlStore = defineStore('hl', (): HlStore => {
  ...
  /**
   * The code the OAuth callback came back with — carried in the store rather than
   * on the dashboard URL, so the address bar stays clean and the dashboard never
   * has to know about OAuth.
   */
  const lastError = ref<string | null>(null)
  /**
   * On demand, never on mount: three HighLevel calls on every dashboard visit
   * spends a rate-limit budget to answer a question nobody asked.
   */
   ...
   *
   * **Every write that lands after an `await` is guarded by this.** Signing out is a
   * route change rather than a page load, so this store outlives the session and a
   * request still in flight resolves against the *next* person's screen. For the
   * probe that is the previous account's CRM counts rendered under someone else's
   * connection, with `probe` left at `ready` so nothing ever clears them.
   */
  let generation = 0
  ...
  // connected status, and the union makes that a compile error instead of a
  // silent `undefined`.
  ...
  /**
   * What to call the connected location — falling back to the id, which is what
   * happens when `locations.readonly` is absent or the lookup failed. A working
   * connection with a worse label, not a broken one.
   */
  const label = computed(() => {
    const current = status.value
    if (current?.connected !== true) return null
    // No `?? null` tail: the union guarantees a connected status has a locationId.
  ...
  /**
   * Leaves the SPA. Nothing after the assignment runs, and `busy` is never cleared
   * on the success path on purpose — the button must stay disabled while the
   * browser navigates away, or a second click mints a second state.
   */
   ...
   * `all` would throw away two working counts because a third failed. The section is
   * only in an *error* state when every one of them failed.
   */
  async function checkDataAccess(): Promise<void> {
    ...
    // The session that asked for this may have ended while it was in flight, and
    // these two lines are the ones that would show its data to whoever signed in
    // next.
  ...
  /**
   * A connection belongs to one account, and signing out is a route change rather
   * than a page load — so without this the next person to sign in on the same
   * browser sees the last one's CRM location named in the panel. `status` matters as
   * much as the rest: left non-null it suppresses the loading state that would hide
   * the stale value.
   *
   * Clearing the refs is only half of it; bumping `generation` is the other half.
   */
  function reset(): void {
    generation += 1
    status.value = null
