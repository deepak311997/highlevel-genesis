 ...
 *
 * The failure mode this exists for is **silent**: an unmapped extension renders
 * as plaintext, which looks like a file that simply has no colours rather than
 * like a bug. Pinning the map to the server's allowlist is what makes a future
 * extension addition fail a test instead of shipping colourless.
 *
 * `.json` mapping to plaintext is a **decision, not an omission** (D4): monaco
 * ships no `basic-languages/json`, because JSON's tokenizer lives only inside the
 * worker-backed language service — and that service is the one thing D3 refuses,
 * since it puts red squiggles on LLM-generated code.
 */

/**
 * `functions/src/files/schema.ts`'s `FILE_EXTENSIONS`, restated.
 *
 * The functions package is not reachable from `frontend/`, which is why
 * `filesApi.spec.ts` restates `FILE_BYTES_MAX` the same way. Restated rather than
 * derived: the point is that adding an extension there without deciding what
 * colours it here fails a test.
 */
...
describe('editorLanguage', () => {
  ...
  /*
   * The whole allowlist, so an extension added on the server cannot arrive here
   * uncoloured and unnoticed. Asserted on the map's own keys rather than on
   * `editorLanguage`'s answer: every extension returns *something*, so a call
   * alone cannot tell a deliberate mapping from a fall-through to the default.
   */
