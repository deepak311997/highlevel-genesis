...
/**
 * AC-6, D4 — **a toast may carry only the transient outcome of an action the user just took,
 * that leaves nothing on screen to read.**
 *
 * Every failure in this app stays in the inline surface it already has. An error that vanishes
 * after four seconds is an error the user cannot act on: they cannot re-read it, cannot copy it,
 * and cannot find it again once it has gone — and this app's failures all have somewhere to live
 * already, next to the thing that failed and with the retry that answers them. That is why
 * adopting the toast library was a two-call-site change rather than a rewrite of eighteen error
 * surfaces.
 */
...
/**
 * Built by concatenation, and every `*.spec.ts` is skipped below — this file with them.
 *
 * Both halves are needed for a scanner that does not trip on its own source, or on the doubles
 * the component specs stand up for it. A sibling scan went red once because a sentence quoted
 * its own needle; a needle in two pieces cannot.
 */
 ...
 *
 * The vendored region is imported by its own path and is the fix, not the offence; and an inline
 * error surface is what D4 asks for, so the components that render one have to stay silent here.
 */
...
/**
 * The vendored region's own import specifier, and the one file allowed to name it.
 *
 * A second needle because the first one cannot see this: a view that mounted its own `<Toaster
 * />` would import `@/components/ui/sonner`, which does not contain `vue-sonner` at all —
 * `INNOCENT` below even names that import as the thing the first scan must stay quiet about.
 * `App.spec.ts` counts the regions it can see, but it stubs the routed views, so a `Toaster`
 * inside `DashboardView.vue` is invisible there too. Between them that left AC-7 with no test
 * that a second region cannot exist, and two regions render every toast twice.
 */
...
/**
 * Skipped so the scanner does not trip on its own source, nor on the component specs that mock
 * the module to assert a call site behaves — a test double is not a call site.
 */
 ...
 *
 * Split out so the cases below exercise the predicate the tree walk uses. Asserting
 * `source.includes(NEEDLE)` inline instead would be a tautology — the fixtures are built by
 * interpolating `NEEDLE` — and a tautology proves nothing about the scanner it stands in for.
 * `no-cdn.spec.ts` calls its own `hits()` here for the same reason.
 */
...
describe('the frontend source tree', () => {
  ...
  /*
   * AC-7, and the half `App.spec.ts` cannot reach. It counts `Toaster`s in a tree whose routed
   * views are stubs, so it would still read one with a second region sitting in
   * `DashboardView.vue`.
   */
