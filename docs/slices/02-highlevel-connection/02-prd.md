...
| D25 | The install produces a `Company` token — reject it, or resolve it? | **Resolve it, when it is unambiguous.** A `Company` token whose install covers exactly one sub-account is traded for a location token via the documented `installedLocations` → `locationToken` path. Zero or many → `wrong_account_type`. | Installing from the marketplace listing installs **agency-wide** — `isBulkInstallation: true`, no `locationId` — and that is the install a user can actually perform; rejecting it outright would refuse the real flow. But an install spanning several sub-accounts has no answer to "which location did the user mean", and picking the first would bind a Genesis project to an arbitrary client's CRM, which is the tenant-isolation mistake this slice exists to avoid. So the unambiguous case is resolved and the ambiguous one is refused. |
| D26 | Which scopes does the app actually grant? | **Nine, not fourteen.** HighLevel refused `conversations.write`, `users.readonly`, `opportunities.readonly`, `locations/customFields.readonly` and `locations/tags.readonly` with `invalid_scope`. | None is required by the spec: F7.1 asks for Contacts, Conversations and Calendars and all three surfaces remain covered. `conversations.write` governs conversation *records*; F7.1's "send" is `conversations/message.write`, which is kept. The rest were the "worth adding cheaply" extras. |
...
Auth: Firebase ID token (`Authorization: Bearer`) **and** `email_verified` true (Slice 1's D26). App Check required.
...
| Authenticated but unverified | 403 — the gate holds for non-browser callers too (Slice 1's D26) | n/a |

## Acceptance criteria

**Connect**

- **AC-1** — Given a verified signed-in user, when they `POST /api/hl/connect`, then the response is 200 and `authorizeUrl` points at `HL_AUTHORIZE_BASE + /v2/oauth/chooselocation`. *(Amended during the build: the v1 path answers `No integration found with the id: <app id>` against a live app.)*
- **AC-2** — Given that response, then `authorizeUrl` carries `response_type=code`, the configured `client_id`, **`version_id`**, `redirect_uri` equal to `HL_REDIRECT_URI` byte for byte, `loginWindowOpenMode=self`, and a `state`.
...
- **AC-16** — Given a token response whose `userType` is `Company`, when its install covers **exactly one** location, then that location is resolved through `installedLocations` + `locationToken` and the connection is stored as normal. Given it covers **zero or more than one**, then 302 to `?code=wrong_account_type` and no connection is written. *(Amended during the build — see D25.)*
...
3. **Rotation-on-use — ~~high~~ measured and downgraded.** This risk read "two parallel refreshes brick the connection permanently", and D13 was argued on it. Measured against the sandbox: the old refresh token **survives at least one reuse** and only fails on the third attempt, so HighLevel runs a grace window rather than strict rotation-on-use. A concurrent double refresh is survivable. The transactional refresh remains worth building — it prevents wasted calls and inconsistent stored state — but it is no longer the only thing between us and an unrecoverable connection, and on that basis **T13 was cut to Slice 8** where it gets its first real consumer.
