...
| D7 | What sends the email? | ~~**SMTP2GO REST API**, ported from VoiceSquad.~~ **Reversed during the build — see D31.** Firebase Authentication sends its own email; no provider, no secret, no sender domain. | Already a proven integration the user operates; one API key, no SMTP configuration, trivial to stub from a fixture. |
| D8 | Does password reset stay client-side? | ~~**No — it moves to the same function.**~~ **Reversed during the build — see D31.** It stays client-side, because Firebase sends it and enumeration protection makes it non-disclosing. | The "you already have an account" email *is* a reset link. Leaving forgot-password on Firebase's client-side template would send two differently-branded emails for one action. Consolidating also makes reset non-disclosing for free. |
...
| D23 | Password policy, and where enforced? | ~~**NIST SP 800-63B aligned:** minimum 8, at least 64 accepted, no composition rules.~~ **Reversed during the build — see D30.** Now: minimum 8, maximum 50, uppercase + lowercase + numeric + special all required, mirroring the Identity Platform policy configured on the project. | Passwords are set through Identity Toolkit, so client validation alone is bypassable by calling the API directly. Server-side enforcement is the only real one. Note: the password-policy setting is an Identity Platform feature — confirm the project's tier during build (open risk 7). Validation runs before any Auth call, so a weak-password rejection never depends on whether the address exists. |
...
Recorded here so the discovery document does not describe a design that was
superseded once the platform was measured.

| # | Question | Decision | Rationale |
|---|---|---|---|
| D29 | **D18a is not implementable.** Does Firebase retire a superseded verification code? | **No — and neither does deleting the account.** So the rule becomes: **a registration request never changes anything about an account that already exists.** It only mails a reset link, which is useful solely to whoever holds the mailbox. | Measured against the emulator: a code minted for a *deleted* uid still applied and verified the replacement account — codes resolve by address, not by uid. The replacement rule is stronger than D18a on the point that matters, because a registration can never alter an account it does not control. Both orderings of the pre-hijack then end safely. Pinned by a `platform behaviour` test so it fails loudly if Firebase ever changes. |
| D31 | Does Genesis need a mail provider at all? | **No.** Firebase Authentication sends verification and password-reset email itself, from `noreply@<project>.firebaseapp.com`, with templates editable in the console and no domain to verify. SMTP2GO, the transport abstraction, the templates and `_devMail` are all deleted, along with `/auth/resend` and `/auth/password-reset`. | D7 and D8 were chosen when the "you already have an account" email meant we had to send *something* ourselves. Dropping that one email removes the entire provider. The consequence: registration cannot send the first verification email — it runs through the Admin SDK, which only generates links, and Firebase's sender needs a signed-in `currentUser` — so the gate sends it after sign-in. **This strengthens D5 rather than weakening it:** previously the branches sent different emails, so the branch was observable to whoever held the mailbox; now nothing differs at all. It also means registering someone else's address mails them nothing, so the endpoint cannot be used to send unsolicited email. Cost: no "you already have an account" nudge, and unbranded sender. |
| D30 | Password policy — whose definition wins? | **The Firebase console's.** Minimum 8, maximum 50, all four composition classes required. Mirrored in `functions/src/auth/schema.ts` and `frontend/src/lib/password.ts`. | D23 chose NIST, and that guidance still holds in general — composition rules push people toward `P@ssw0rd1`, and length is what resists guessing. But the console policy governs `confirmPasswordReset`, which runs client-side, so a divergence would let someone sign up with a password they could never set again on reset. The project owner chose the stricter-looking policy; the code follows the console rather than fighting it. |
